...
;(function() {
  ...
  function _fmtToFT_IN_DEC(base, digits, sfx) {
    ...
    return _saneInput(function (valueIn) {
      ...
      // If feet === 0 then only return inches and make sure we have the correct sign
      return feet !== 0 ? feet + sfx[0] + Math.abs(inches.toFixed(digits)) + sfx[1] : (Math.abs(inches.toFixed(digits)) * sign) + sfx[1];
    });
  }
  function _fmtToFT_IN_FRAC(base, denom, sfx) {
    ...
    return _saneInput(function (valueIn) {
      ...
      if (numerator === 0) {
        if (feet === 0) { // Return only inches and ensure correct sign
          return (sgn + Math.abs(wholeInches) + sfx[1])
        } else {
      ...
      if (feet === 0) { // Return only inches and ensure correct sign
        return (sgn + Math.abs(wholeInches) + ' ' + Math.abs(numerator) + '/' + denom + sfx[1])/*.replace(/(.)\1+/g, '$1');*/
      } else {
